...
非常重要的判定规则（必须遵守）：
1. 仅基于提供文本比对，不得假设图片、音频、教师用书中的额外信息。
2. 证据不足时，status 必须用 uncertain，不要强行判 mismatch。
3. criticalIssues 只允许收录“证据明确且影响作答正确性”的问题。
4. 每条 mismatch/criticalIssues 都要在 note/issue 中附简短证据片段（不超过20词）。
